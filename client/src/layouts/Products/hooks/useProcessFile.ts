...
const useProcessFile = () => {
   ...
   const validateAndTransformRow = (
      row: any,
      index: number
   ): { product?: ProductInt; error?: string } => {
      try {
         ...
         const brandValue = cleanValue(row['Marca']);
         const satValue = cleanValue(row['Codigo SAT']);
         ...
         const details: DetailsProduct = {
            brand: String(brandValue),
            satCode: String(satValue),
         };

         const prices: PricesProductInt = {
         ...
         const product: ProductInt = {
            code: String(keyValue),
            key: String(claveValue),
   ...
   const processFile = useCallback((file: File) => {
      ...
      reader.onload = (e) => {
         try {
            ...
            const validProducts: ProductInt[] = [];
